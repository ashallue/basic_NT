...
// 64-bit strong pseudoprime test for the given base
...
// 64-bit fermat pseudoprime test for the given base
// this one returns true if base^{n-1} = 1 mod n
bool fermat1(int64 base, int64 n){
    int128 x = powmod(base, n-1, n);
    return x == 1;
}

// this one returns true if base^n = base mod n
bool fermata(int64 base, int64 n){
  int128 x = powmod(base, n, n);
  return x == base;
}
